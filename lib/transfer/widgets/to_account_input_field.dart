...
class ToAccountInputField extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final toAccount = context.select((TransferBloc bloc) => bloc.state.toAccount);
    final fromAccount = context.select((TransferBloc bloc) => bloc.state.fromAccount);
    return DropdownButtonFormField<Account>(
        ...
        items: budget.accountList
          ...
          return DropdownMenuItem(
            value: account,
            child: Text(account.extendName(budget.getCategoriesByType(TransactionType.ACCOUNT))),
          );
        }).toList(),
        onChanged: (newValue) {
          context
